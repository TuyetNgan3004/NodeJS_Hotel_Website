...
function AddSerive(name, type, price, qty) {
	...
	`
		...
		service_row.querySelector('.sv-qty .qty-input').addEventListener('click', function() {
			...
			// sv_total = sv_total.replace(/,/g, '');
			var price = service_row.querySelector('.price').innerText;
			price = price.replace(/,/g, '');
			var total = 0;
			total = parseFloat(price) * parseFloat(input_value);
...
var sv_table = document.getElementById('service-container').addEventListener('change', function() {
	var sv_price = document.querySelectorAll('.sv-total');
	console.log(sv_price);
});
// var total = totalHTML.value;
// total = parseFloat(total.replace(/,/g, ''));
// total += sv_price;
// totalHTML.innerHTML = 'hello' + Intl.NumberFormat().format(total);
