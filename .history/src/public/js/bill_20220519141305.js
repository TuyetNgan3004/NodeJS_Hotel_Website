...
add_btn.forEach(btn => {
	btn.addEventListener('click', function(e) {
		...
		console.log(service.querySelector('.sv-name').innerText)
		const item = {
			name: service.getElementsByClassName('.sv-name').innerText,
			type: service.getElementsByClassName('.sv-type').innerText,
			price: service.getElementsByClassName('.sv-price').innerText,
		}
		console.log(item);
	})
})
