...
function AddSerive(name, type, price, qty) {
	...
	serviceContent += `
		...
		<td>${type}</td>
		<td>${price}</td>
		<td class="sv-qty"><input onchange="updateServiceTotal" class="qty-input" type="number" min="1" value="${qty}" name="" id=""></td>
		<td>${result}</td>
		<td style="min-width: 0;">
			<div class="status-btn-circle service-remove"> <i class="fas fa-minus"></i></div>
		</td>
	`
		...
		service_row.querySelector('.sv-qty .qty-input').addEventListener('onchange', function() {
	...
	// var tb_service_qty = serviceContainer.querySelectorAll('.sv-qty');
