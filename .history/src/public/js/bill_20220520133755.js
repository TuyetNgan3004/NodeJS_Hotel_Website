...
add_btn.forEach(add => {
	add.addEventListener('click', function(e) {
		...
		if(service.tagName == 'TR') {
			...
			var sv_qty = service.getElementsByClassName('svList-qty')[0].value;
			console.log(service.getElementsByClassName('svList-qty')[0]);
...
function AddSerive(name, type, price) {
	...
	for(var i = 0; i < serviceRow.length; i++) {
		...
		var item_qty = serviceRow[i].getElementsByClassName('sv-qty')[0].value;
		if(item_title) {
			if(item_title === name) {
